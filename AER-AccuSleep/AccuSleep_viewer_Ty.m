...
G.colors = [1 1 1; 1 0.788 0.2; 1 0.7 0.7; 0.7 0.7 1; 0.9 0.6 0; 0.7 0.1 0; 0.2 0.2 0.6];
...
G.A1 = axes('Units', 'Normalized', 'Position', [0.05 0.85 0.87 0.05]);
...
% EEG spectrogram
G.A3 = axes('Units', 'Normalized', 'Position', [0.05 0.92 0.87 0.06]);
G.A3.Toolbar.Visible = 'off';
set(gca, 'FontSize', 10, 'LineWidth', 2, 'XTick', [], 'YTick', []);
ylabel(G.A3, 'Spec.');

% EEG signal - 1 Minute
G.A6a = axes('Units', 'Normalized', 'Position', [0.05 0.76, 0.87 .09]);
G.A6a.Toolbar.Visible = 'off';
ylabel(G.A6a, 'EEG1');
set(G.A6a,'XTick',[])

% % EMG signal - 1 Minute
G.A7a = axes('Units', 'Normalized', 'Position', [0.05 0.66 0.87 .09]);
G.A7a.Toolbar.Visible = 'off';
ylabel(G.A7a, 'EMG1');
set(G.A7a,'XTick',[])

% EEG signal  - main
G.A6 = axes('Units', 'Normalized', 'Position', [0.05 0.56 0.87 .09]);
G.A6.Toolbar.Visible = 'off';
ylabel(G.A6, 'EEG2');

% EMG signal  - main
G.A7 = axes('Units', 'Normalized', 'Position', [0.05 0.45 0.87 .09]);
G.A7.Toolbar.Visible = 'off';
ylabel(G.A7, 'EMG2');
set(G.A7,'XTick',[])

% EEG signal + 1 minute
G.A6b = axes('Units', 'Normalized', 'Position', [0.05 0.35 0.87 .09]);
G.A6b.Toolbar.Visible = 'off';
ylabel(G.A6b, 'EEG3');
set(G.A6b,'XTick',[])

% EMG signal + 1 minute
G.A7b = axes('Units', 'Normalized', 'Position', [0.05 0.25 0.87 .09]);
...
G.A10 = axes('Units', 'Normalized', 'Position', [0.45 0.135 0.10 0.09]);
...
%% Functions used by buttons, keypresses, etc.
    function updatePlots(~, ~) % update plots when something changes
        ...
        set(G.A9, 'XTickLabel', [],'XTick',[], 'YTick', [1 2 3], 'YTickLabel', {'Wake', 'NREM', 'REM'});
    ...
    function updateState() % update the sleep stage image
        ...
        set(G.A1, 'XTickLabel', [],'XTick',[], 'YTick', [1 2 3], 'YTickLabel', {'Wake', 'NREM', 'REM'});
    end
    ...
    function showAllEpochsPSD(G)
        ...
        freqRange      = [0 9]; % show 0..20 Hz
        fs             = G.SR;  % sampling rate
...
% Process keypresses
    function keypress(~, evt)
        % Initialize keyPressCount if it doesn't exist
...
% check if a structure x has a properly constructed labels field
    function [allChecksPassed] = checkLabels(x, checkLen)
        ...
        if ~isempty(setdiff(unique(x.labels), 1:6)) % in the range 1:6
